...
const STATUS_STYLES = {
  ...
  not_a_job: 'bg-stone-100 text-stone-600 border-stone-300',
}

const UPCOMING_COLLAPSED_KEY = 'my-task.upcomingCollapsed'
...
export function ApplicationsPage({
  ...
  const [upcomingCollapsed, setUpcomingCollapsed] = useState(() => {
    try {
      return localStorage.getItem(UPCOMING_COLLAPSED_KEY) === '1'
    } catch {
      return false
    }
  })
  ...
  const toggleUpcomingCollapsed = () => {
    setUpcomingCollapsed((prev) => {
      const next = !prev
      try {
        localStorage.setItem(UPCOMING_COLLAPSED_KEY, next ? '1' : '0')
      } catch {
        /* ignore */
      }
      return next
        ...
        {!upcomingCollapsed ? (
          upcoming.length === 0 ? (
            <p className="text-xs text-slate-500 py-4 text-center border border-dashed border-slate-200 rounded-xl">
              No upcoming events. Add an interview, call, or LinkedIn contact linked to an
              application.
            </p>
          ) : (
              ...
              {upcoming.map((ev) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    {ev.notes ? (
                      <p className="text-[11px] text-slate-600 mt-1 line-clamp-2 whitespace-pre-wrap">
                        {ev.notes}
                      </p>
                    ) : null}
                    {ev.link ? (
          ...
          <p className="text-[11px] text-slate-500">
            Collapsed — tap the header to expand. Add events anytime with + Add event.
          </p>
        )}
      </div>
          ...
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="text-[10px] tracking-wider border-b border-slate-100">
                ...
                <th className="p-2.5 font-bold uppercase text-slate-500">Notes</th>
                ...
                filteredApps.map((app) => (
                  ...
                  >
                    ...
                    <td className="p-2.5 text-slate-600 max-w-[220px]">
                      <p className="truncate text-[11px]" title={app.notes || ''}>
                        {app.notes || '—'}
                      </p>
                    </td>
          ...
          >
            ...
            <label className="block text-xs font-semibold text-slate-700">
              ...
              <span className="font-normal text-slate-500">
                {' '}
                — interview progress, why “Not a job”, next steps
              </span>
              <textarea
                value={appForm.notes}
                ...
                rows={5}
                className="mt-1 w-full px-3 py-2 border border-slate-200 rounded-xl text-sm"
                placeholder="e.g. Kenny screen Thu 11:30 · waiting on HM loop · false positive from LinkedIn invite"
              />
            </label>
            {appForm.status === 'not_a_job' ? (
              <p className="text-[11px] text-stone-600 bg-stone-50 border border-stone-200 rounded-lg px-2.5 py-1.5">
                Marked as not a job so we can audit false Gmail imports later. Keep a short reason
                in notes.
              </p>
            ) : null}
            {modalError ? (
              <p className="text-xs text-rose-800 bg-rose-50 border border-rose-100 rounded-lg px-2.5 py-2">
                {modalError}
                {/check|constraint|status/i.test(modalError) ? (
                  <span className="block mt-1 text-[11px] text-rose-700">
                    Likely the DB is missing <code>not_a_job</code> — run the status CHECK update in
                    Supabase SQL (see schema.sql).
                  </span>
                ) : null}
              </p>
            ) : null}
