...
/**
 * Guess company from From display name or subject.
 * @param {{ from?: string, subject?: string }} msg
 */
export function guessCompany(msg = {}) {
  ...
  if (domain) {
    ...
    if (
      root &&
      !['gmail', 'googlemail', 'yahoo', 'outlook', 'hotmail', 'icloud'].includes(root)
    ) {
 ...
 * @returns {{ kind: string, proposed_status?: string, proposed_title?: string, proposed_company?: string|null } | null}
 */
...
/**
 * Match proposal company to an existing application (case-insensitive substring).
 */
